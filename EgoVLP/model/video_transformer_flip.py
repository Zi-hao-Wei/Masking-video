...
class PatchDropout(nn.Module):
    ...
    def forward(self, x, N, L, D):
        ...
        # frame_indices = torch.arange(frame)
...
class PatchDropoutSim(nn.Module):
    ...
    def forward(self, x,sim=0.95,mask_ratio=0.6):
...
class SpaceTimeTransformer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
                 ...
                 num_frames=16, time_init='rand', attention_style='frozen-in-time'):
        """
        Args:
    ...
    def forward_features(self, x):
        ...
        # print("pos",tile_pos_embed.shape)
        # print("temp",self.temporal_embed.shape)
        tile_temporal_embed = self.temporal_embed.repeat_interleave(self.patches_per_frame_origin, 1)
        
        # print("tile",tile_temporal_embed.shape)
...
if __name__ == "__main__":
    network = SpaceTimeTransformer(num_frames=16).cuda()
    data = torch.rand(torch.Size([4, 16, 3, 224, 224])).cuda()
    x = network(data)
    print(x.shape)