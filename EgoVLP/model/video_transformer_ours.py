...
class VarAttention(nn.Module):
    ...
    def forward(self, x, einops_from, einops_to, attn_mask=None,**einops_dims):
        h = self.num_heads
        ...
        if attn_mask is not None:
            _,l,d = attn_mask.shape
            attn_mask = attn_mask.repeat(h,1,1,1)
            attn_mask = attn_mask.reshape(-1,l,d)            
        ...
        # attention
        out = attn(q_, k_, v_,attn_mask=attn_mask)
...
class SpaceTimeBlock(nn.Module):
    ...
    def forward(self, x, einops_from_space, einops_to_space, einops_from_time, einops_to_time,
        ...
        space_output = self.attn(self.norm1(time_residual), einops_from_space,
                                 einops_to_space, attn_mask=attn_mask ,f=space_f)
...
class PatchDropoutSim(nn.Module):
    """
    ...
    def forward(self, x,N,L,D,sim=0.8):
        ...
        x = x.reshape(N_o,-1,D_o)

        if self.exclude_first_token:
            x = torch.cat((cls_tokens, x), dim=1)

        attn_mask_enc = attn_mask_enc[:,1:,:]
        
        return x, new_L, attn_mask_enc, mask_ratio
...
class SpaceTimeTransformer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
        ...
        self.patches_per_frame_origin = num_patches // num_frames
        # print(self.patches_per_frame)
        ...
        self.patch_drop_ours = PatchDropoutSim(prob=0.5)
    ...
    def forward_features(self, x):
        ...
        x = x.flatten(2)
        # print("flattened",x.shape) # 3*()*emb

        x = x.transpose(2, 1)
        # print("transpose",x.shape) # 3*()*emb
        # _,L,N = x.shape
        # x_tmp = x.reshape(b,-1,L,N)
        # print("frames",x_tmp.shape)

        # x_tmp = self.patch_drop(x_tmp)
        # print("frames_drop",x_tmp.shape)
        ...
        tile_temporal_embed = self.temporal_embed.repeat_interleave(self.patches_per_frame_origin, 1)
        ...
        # Patch dropping
        x,new_L, attn_mask_enc, mask_ratio = self.patch_drop_ours(x,N,L,D)
        ...
        for blk in self.blocks:
            x = blk(x, self.einops_from_space, self.einops_to_space, self.einops_from_time,
                    self.einops_to_time,
                    time_n=n, space_f=f,attn_mask=attn_mask_enc)
...
if __name__ == "__main__":
    network = SpaceTimeTransformer(num_frames=4).cuda()
    data = torch.rand((3, 4, 3, 224, 224)).cuda()
    x = network(data)
    print(x.shape)